...
const EditDialog = ({ isOpenEdit, toggleDialogEdit, subs }: IProps) => {
    console.log(subs)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (title.length < 3) {
      toast.error("SubCategory name cannot be less than 3 caharacters!");
      return; // Prevent submission
    }
    setIsLoading(true);
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:1337/api/sub-categories/${subs?.documentId}?populate=*`, formData);
