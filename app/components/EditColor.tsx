...
const EditColor = ({ isOpenEdit, toggleDialogEdit, subs }: IProps) => {
    console.log(subs)
  const [isLoading, setIsLoading] = useState(false);
  const [title, setTitle] = useState(subs?.title || "");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (title.trim() === "") {
      ...
      return; // Prevent submission
           ...
           if (title.length <= 2) {
      toast.error("Color name cannot be less than or equal 2 characters!");
      return; // Prevent submission
    }
    setIsLoading(true);
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:1337/api/colors/${subs?.documentId}?populate=*`, formData);
      ...
      >
        <div className="relative bg-white rounded-lg shadow-lg p-6 w-96 md:w-[400px] lg:w-[500px] transform transition-transform duration-500 ease-in-out">
          ...
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-semibold mb-4 text-gray-800">Edit Color</h2>
            <ButtonIcon
              className="text-gray-500 hover:text-gray-800 transition-colors"
              onClick={toggleDialogEdit} // Close dialog correctly
            >
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4 flex flex-col space-y-2">
              ...
              <input
                type="text"
                id="productName"
                value={title} // Use the state for the title
