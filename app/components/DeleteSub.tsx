...
const DeleteSub = ({ isOpenDelete, toggleDialogDelete, subs }: IProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.delete(`http://localhost:1337/api/sub-categories/${subs?.documentId}`);
      ...
      >
        <div className="relative bg-white rounded-lg shadow-lg p-6 w-96 md:w-[400px] lg:w-[500px] transform transition-transform duration-500 ease-in-out">
          ...
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-semibold mb-4 text-gray-800">Delete Sub-Category</h2>
            <ButtonIcon
              className="text-gray-500 hover:text-gray-800 transition-colors"
              onClick={toggleDialogDelete} // يعمل بشكل صحيح لإغلاق الديالوج
            >
