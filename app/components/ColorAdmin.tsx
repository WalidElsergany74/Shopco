...
const ColorAdmin = ({ colors }: { colors: IProductColor[] }) => {
  const { isOpen, toggleDialog } = useDialog();
  const [isOpenEdit, setIsOpenEdit] = useState(false);
  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [selectedEdit, setSelectedEdit] = useState<IProductColor | null>(null); 
  const [selectedDelete, setSelectedDelete] = useState<IProductColor | null>(null); 
  ...
  const toggleEdit = (documentId: string) => {
    const selectedItem = colors.find((item) => item.documentId === documentId);
    ...
    <>
      <div className="products flex flex-col justify-center p-4">
        <div className="flex flex-col justify-start items-start space-y-3 md:space-y-0 md:flex-row md:justify-between md:items-center mb-6">
          <h1 className="text-3xl font-extrabold tracking-wider">Colors</h1>
          <ButtonIcon
            onClick={toggleDialog}
            className="py-2 px-4 flex items-center justify-center gap-1 bg-slate-800 hover:bg-slate-700 text-white rounded-lg"
          >
            <FaPlus size={12} />
            <p className="text-base font-semibold">Create Color</p>
          </ButtonIcon>
        </div>
        {colors.length === 0 ? (
          <p>No Colors Created Yet</p>
        ) : (
      ...
      <AddColor toggleDialog={toggleDialog} isOpen={isOpen} />
