...
const Drawer = ({ openRight, toggleDrawer }: IDrawer) => {
  ...
  const handleUpdateQuantity = async (itemId: string, change: number) => {
    try {
      ...
      const productRes = await axios.get(`http://localhost:1337/api/products/${existsItem.productId}`);
      ...
      if (updatedQuantity === 0) {
        await axios.delete(`http://localhost:1337/api/cart-items/${existsItem.documentId}`);
      } else {
        await axios.put(`http://localhost:1337/api/cart-items/${existsItem.documentId}`, {
  ...
  const handleRemoveItem = async (itemId: string) => {
    try {
      ...
      await axios.delete(`http://localhost:1337/api/cart-items/${itemId}`);
