...
const AddProductDialog = ({ isOpen, toggleDialog  }: IProps) => {
    ...
    const handleRemoveImage = (e: React.MouseEvent<HTMLButtonElement>, imageKey: string) => {
      ...
      const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
          ...
          const response = await axios.post("http://localhost:1337/api/categories?populate=*", formData);
  ...
  {preview ? (
    <div className="relative h-full w-full">
      <img src={preview} alt="Image 1" className="h-full w-full object-cover rounded-lg" />
      <button
